...
void A_output(struct msg message)
{
    ...
    {
        printf("  A_output: the message has not ack, so it will drop the message: %s\n", message.data);
        return;
    }
...
void A_input(struct pkt packet)
{
    if (A.state != WAIT_ACK)
    {
        printf("  A_input: A to B only. drop the message.\n");
        return;
    }
    if (packet.checksum != get_checksum(&packet))
    {
        printf("  A_input: the packet is  corrupted. drop the message .\n");
        return;
    }

    if (packet.acknum != A.seq)
    {
        printf("  A_input: this is not the expected Acknolodgment . drop the message .\n");
        return;
    }
    printf("  A_input: ack ok .\n");
    stoptimer(0);
    A.seq = 1 - A.seq;
    A.state = WAIT_LAYER5;
}
...
void A_timerinterrupt(void)
{
    if (A.state != WAIT_ACK)
    {
        printf("  A_timerinterrupt: i'm not waiting for ACK.  so will ignore the  event.\n");
        return;
    }
...
void B_input(struct pkt packet)
{
    if (packet.checksum != get_checksum(&packet))
    {
        printf("  B_input: the packet  is corrupted, so NACK will be sent.\n");
        send_ack(1, 1 - B.seq);
        return;
    }
    if (packet.seqnum != B.seq)
    {
        printf("  B_input: this is not the expected sequenceso NACK will be sent.\n");
        send_ack(1, 1 - B.seq);
        return;
    }
    printf("  B_input: recieve message: %s\n", packet.payload);
