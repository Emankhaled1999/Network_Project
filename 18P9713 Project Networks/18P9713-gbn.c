...
void send_window(void)
{
    while (A.nextseq < A.buffer_next && A.nextseq < A.base + A.window_size)
    {
        ...
        printf("  send_window:the packet will be sent  (seq=%d): %s\n", packet->seqnum, packet->payload);
        tolayer3(0, *packet);
        if (A.base == A.nextseq)
...
void A_output(struct msg message)
{
    ...
    {
        printf("  A_output:the  buffer is full so the message will be dropped : %s\n", message.data);
        return;
    }
...
void A_input(struct pkt packet)
{
    ...
    {
        printf("  A_input:the  packet iss corrupted. drop.\n");
        return;
    }
...
void B_input(struct pkt packet)
{
    if (packet.checksum != get_checksum(&packet))
    {
        printf("  B_input: the packet is corrupted so NACK will be sent  (ack=%d)\n", B.packet_to_send.acknum);
        tolayer3(1, B.packet_to_send);
        return;
    }
    if (packet.seqnum != B.expect_seq)
    {
        printf("  B_input: this is not the expected sequence so NACK will be sent (ack=%d)\n", B.packet_to_send.acknum);
        tolayer3(1, B.packet_to_send);
        return;
    }

    printf("  B_input: the packet is  recvieved  (seq=%d): %s\n", packet.seqnum, packet.payload);
